...
if (!class_exists('Themify_Builder')) :
	...
	public function load_current_inner_page_content($builder_data, $page_breaks) {
	    $page = $paged = !empty($_GET['tb-page']) ? (int) $_GET['tb-page'] : 1;
	    $page=(int)$page;
	    $temp_data = array();
	    $page_num = 1;
	    foreach ($builder_data as $row) {
	    ...
	    $numpages = $page_breaks;
		$return['pagination'] = $this->page_break_pagination($numpages);
	...
	private function count_page_break_modules($post_id) {
		...
		/**
		 * Generate Page break pagination
		 */
		private function page_break_pagination($total=1) {
			$paged = !empty($_GET['tb-page']) ? (int) $_GET['tb-page'] : 1;
			$max_page = $total+1;
			$out = '';
			...
			if ($max_page > 1) {
				$out .= '<div class="post-pagination pagenav clearfix">';
				for ($i = $start_page; $i <= $end_page; $i++) {
					if ((int) $i === (int) $paged) {
						$out .= ' <span class="number current">' . $i . '</span> ';
					} else {
						$out .= ' <a href="' . esc_url(add_query_arg(array('tb-page' => $i))) . '" class="number">' . $i . '</a> ';
					}
				}
				$out .= '</div>';
			}
